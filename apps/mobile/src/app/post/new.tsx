...
export default function NewPostScreen() {
  ...
  const [scope, setScope] = useState<'school' | 'all'>('school'); const [anonymous, setAnonymous] = useState(false); const [submitting, setSubmitting] = useState(false);
  ...
  return (
    <Screen>
      <AppHeader eyebrow="NEW POST" title="경험을 나눠 주세요" action={<Pressable onPress={() => router.back()}><Text style={styles.close}>닫기</Text></Pressable>} />
      ...
      <Card style={styles.options}>
        ...
        <Pressable style={styles.switchRow} onPress={() => setAnonymous((value) => !value)}><View style={[styles.checkbox, anonymous && styles.checkboxOn]}><Text style={styles.check}>{anonymous ? '✓' : ''}</Text></View><View><Text style={styles.anonymousTitle}>익명으로 게시</Text><Text style={styles.anonymousBody}>운영자에게는 안전 관리를 위해 계정 정보가 보입니다.</Text></View></Pressable>
      </Card>
