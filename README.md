...
| 온보딩 | 학교, D-2/D-4, 닉네임, 답변 언어 설정 | 구현 |
...
데모 모드의 게시글, 좋아요, 북마크, 신고, 차단 상태는 테스트용 메모리 데이터입니다. 실제 영구 저장과 멀티 사용자 동작은 Supabase 운영 모드에서 연결해야 합니다.
...
3. 닉네임, 학교, D-2/D-4, 답변 언어를 선택합니다.
4. Today 화면에서 체크리스트 완료 상태를 변경합니다.
5. 커뮤니티에서 내 학교/전체와 카테고리 필터를 바꿉니다.
6. 새 글을 작성하고 피드에 즉시 추가되는지 확인합니다.
7. 다른 사용자의 글 상세에서 신고·차단 진입점을 확인합니다.
8. 공식정보 탭에서 질문하고 답변·체크리스트·출처가 표시되는지 확인합니다.
9. 내 정보에서 로그아웃과 계정 삭제 흐름을 확인합니다.
