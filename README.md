...
| 내 정보 | 프로필 편집, 데모 데이터 초기화, 로그아웃, 계정 및 데이터 삭제 | 구현 |
| 백엔드 | PostgreSQL 스키마, RLS, seed, Edge Functions | 구현 |
| 배포 | Expo/EAS 개발·미리보기·운영 프로필 | 설정 완료 |

데모 모드의 게시글, 댓글, 좋아요, 북마크, 신고, 차단, 개인 일정과 프로필은 기기에 저장되어 새로고침 후에도 유지됩니다. 운영 모드에서는 같은 동작이 Supabase와 RLS를 통해 사용자별로 저장됩니다. 내 정보의 **데모 데이터 초기화**는 로그인과 프로필을 유지한 채 커뮤니티·일정만 초기 상태로 되돌립니다.
...
12. 내 정보에서 데모 데이터 초기화, 로그아웃, 계정 삭제 흐름을 확인합니다.
